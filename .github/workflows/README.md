...
- 对本次更新到的所有包执行 `guix build -L modules <package>`；包名后缀不作为跳过条件
...
`auto-update.yml` 会读取 `scripts/check-updates/report.json`，挑出：

- 本次 `status == "updated"` 的包
- 不按 `-bin` 后缀过滤；闭源和预编译包同样需要验证
