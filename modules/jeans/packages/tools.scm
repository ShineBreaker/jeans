...
;;; Pi: terminal-based coding agent with multi-model support.
;;;
;;; The upstream release is a tar.gz containing:
;;;   - pi               (~111 MB, Bun-embedded ELF binary, dynamically linked,
;;;                        zero NEEDED entries — only the ELF interpreter matters)
;;;   - photon_rs_bg.wasm (WebAssembly image processing module)
;;;   - node_modules/     (native addon: clipboard.linux-x64-gnu.node)
;;;   - assets/, docs/, examples/, export-html/, theme/
;;;
;;; The binary resolves resources relative to /proc/self/exe's directory,
;;; so all ancillary files must live alongside the binary.
;;; Strategy: install everything to lib/pi-bin/, create a bin/ wrapper
;;; that invokes pi through Guix's ld-linux (same pattern as oh-my-pi-bin).

(define-public pi-bin
  (package
    (name "pi-bin")
    (version "0.78.0")
    (source
    ...
    (arguments
     (list
      ...
      #~(modify-phases %standard-phases
          ...
          (replace 'unpack
            (lambda _
              (let ((srcdir (string-append "pi-" #$version)))
                (mkdir srcdir)
                (with-directory-excursion srcdir
                  (invoke "tar" "xzf" #$source))
                (chdir srcdir))))
              ...
              (let* ((out #$output)
                     ...
                     (lib-path (string-join
                                (list (string-append (assoc-ref inputs "glibc") "/lib")
                                      (string-append (assoc-ref inputs "gcc") "/lib"))
                                ":")))
                (copy-recursively "pi" lib)
                (chmod (string-append lib "/pi") #o755)
                (mkdir-p bin)
                (call-with-output-file (string-append bin "/pi")
                  (lambda (port)
                    (format port
                            "#!~a\nexec ~a --argv0 ~a/pi --library-path ~a ~a/pi \"$@\"\n"
                            #$(file-append bash-minimal "/bin/sh")
                            ld.so
                            lib
                            lib-path
    ...
    (synopsis "Terminal-based coding agent with multi-model support")
    (description "Pi is a terminal-based coding agent that supports multiple
LLM providers, mid-session model switching, and a simple CLI for headless
coding tasks.  It features 40+ built-in tools, 13 LSP operations, 27 DAP
operations, subagents, web search, browser automation, and more.
This package provides the prebuilt binary release.")
    (license license:expat)
    (supported-systems '("x86_64-linux"))))
